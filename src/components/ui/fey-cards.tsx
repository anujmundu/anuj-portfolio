...
export const FeyCards = ({
  ...
  return (
    <div className="flex h-[36rem] w-full items-center justify-center bg-black/50 rounded-2xl border border-white/10 relative overflow-hidden my-8">
      <div>
        <motion.h3
          key="solid"
          initial={{ opacity: 0 }}
          ...
          className={cn(
            "absolute top-12 left-1/2 z-50 mx-auto w-fit -translate-x-1/2 text-center text-lg font-bold tracking-tight whitespace-nowrap md:text-3xl",
            "bg-clip-text py-2 text-transparent transition-all duration-500",
        ...
        <motion.div
          className="relative flex h-96 w-80 md:w-96 mask-b-from-10% mt-12"
        ...
        >
          <img
            ...
            width={1000}
            height={1000}
            className="absolute inset-y-0 left-0 h-96 w-40 object-contain"
          />
          ...
          {HERO_CARDS.map((card, index) => {
            ...
            return (
              <motion.div
                key={card.activeSrc}
                className={`group absolute -bottom-2 ${card.left} z-20 h-96 w-36 cursor-pointer ${card.className ?? ""}`}
